...
[cite_start]Цель работы — изучение структуры HTML-документов и реализация семантической разметки[cite: 1, 5]. В отличие от классических статических страниц, данный проект построен как **SPA (Single Page Application)** на React, что соответствует современным стандартам IT-индустрии и требованиям стажировки в **Modsen**.

### Основные разделы:
* [cite_start]**Стартовая страница:** Техническая информация о работе (название, цель, ФИО автора).
...
* [cite_start]**Фотоальбом:** Интерактивное слайд-шоу с автоматическим переключением изображений (интервал 3 сек) и подписями.
* **Стажировка:** Описание текущего опыта JS-разработки в компании Modsen.
* [cite_start]**Карта сайта:** Визуальная интерактивная схема архитектуры проекта, реализованная на SCSS.
...
3.  [cite_start]**Графическая навигация:** Реализация структуры переходов в виде схемы с использованием CSS-псевдоэлементов `::before` и `::after`.
